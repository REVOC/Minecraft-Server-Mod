...
public class PluginLoader {
	...
	public void loadPlugins() {
		if (loaded)
			return;
		log.info("VhMod: Loading plugins...");
		...
		log.info("VhMod: Loaded " + plugins.size() + " plugins.");
		loaded = true;
	}
		...
		try {
			...
			if (!file.exists()) {
				log.log(Level.SEVERE,
						"Failed to find plugin file: plugins/" + fileName + ".jar. Please ensure the file exists");
				return false;
			}
	...
	public Object callHook(Hook h, Object... parameters) {
		...
		synchronized (lock) {
			...
			try {
				...
				for (PluginRegisteredListener regListener : registeredListeners) {
					...
					try {
						...
						case CHAT:
							...
							break;
						case EMOTE:
							if (listener.onEmote((Player) parameters[0], (String) parameters[1]))
								toRet = true;
							break;
						case COMMAND:
						...
						case KICK:
							...
							break;
						case BLOCK_CREATED:
							if (listener.onBlockCreate((Player) parameters[0], (Block) parameters[1],
									(Block) parameters[2], (Integer) parameters[3]))
								toRet = true;
							break;
						case BLOCK_DESTROYED:
							...
							break;
						case PLAYER_MOVE:
							listener.onPlayerMove((Player) parameters[0], (Location) parameters[1],
						...
						case ITEM_PICK_UP:
							...
							break;
						case TELEPORT:
							if (listener.onTeleport((Player) parameters[0], (Location) parameters[1],
									(Location) parameters[2]))
								toRet = true;
							break;
						case BLOCK_BROKEN:
						...
						case IGNITE:
							if (listener.onIgnite((Block) parameters[0],
									(parameters[1] == null ? null : (Player) parameters[1])))
								toRet = true;
							break;
						case EXPLODE:
						...
						case MOB_SPAWN:
							...
							break;
						case DAMAGE:
							if (listener.onDamage((DamageType) parameters[0], (BaseEntity) parameters[1],
									(BaseEntity) parameters[2], (Integer) parameters[3]))
								toRet = true;
							break;
						case HEALTH_CHANGE:
							if (listener.onHealthChange((Player) parameters[0], (Integer) parameters[1],
									(Integer) parameters[2]))
								toRet = true;
							break;
						case REDSTONE_CHANGE:
							toRet = listener.onRedstoneChange((Block) parameters[0], (Integer) parameters[1],
									(Integer) toRet);
							break;
						case BLOCK_PHYSICS:
						...
						case VEHICLE_UPDATE:
							...
							break;
						case VEHICLE_DAMAGE:
							if (listener.onVehicleDamage((BaseVehicle) parameters[0], (BaseEntity) parameters[1],
									(Integer) parameters[2]))
								toRet = true;
							break;
						...
						case VEHICLE_POSITIONCHANGE:
							listener.onVehiclePositionChange((BaseVehicle) parameters[0], (Integer) parameters[1],
									(Integer) parameters[2], (Integer) parameters[3]);
							break;
						case ITEM_USE:
							if (listener.onItemUse((Player) parameters[0], (Block) parameters[1], (Block) parameters[2],
									(Item) parameters[3]))
								toRet = true;
							break;
						case BLOCK_RIGHTCLICKED:
							listener.onBlockRightClicked((Player) parameters[0], (Block) parameters[1],
									(Item) parameters[2]);
							break;
						case BLOCK_PLACE:
							if (listener.onBlockPlace((Player) parameters[0], (Block) parameters[1],
									(Block) parameters[2], (Item) parameters[3]))
								toRet = true;
							break;
						case LIQUID_DESTROY:
							HookResult ret = listener.onLiquidDestroy((HookResult) toRet, (Integer) parameters[0],
									(Block) parameters[1]);
							if (ret != HookResult.DEFAULT_ACTION && (HookResult) toRet == HookResult.DEFAULT_ACTION)
								toRet = ret;
							break;
						case ATTACK:
							if (listener.onAttack((LivingEntity) parameters[0], (LivingEntity) parameters[1],
									(Integer) parameters[2]))
								toRet = true;
							break;
						case OPEN_INVENTORY:
				...
				log.log(Level.SEVERE, "Exception while calling plugin function in '" + listenerString
						+ "' while calling hook: '" + h.toString() + "'.", ex);
	 ...
	 */
	public PluginRegisteredListener addListener(Hook hook, PluginListener listener, Plugin plugin,
