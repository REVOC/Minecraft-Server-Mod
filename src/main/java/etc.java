...
public class etc {
	...
	private String usersLoc = "users.txt", kitsLoc = "kits.txt", homeLoc = "homes.txt", warpLoc = "warps.txt",
			itemLoc = "items.txt", groupLoc = "groups.txt";
	...
	public final void load() {
		...
		try {
			...
			if (monsters.length == 1 && (monsters[0].equals(" ") || monsters[0].equals("")))
				monsters = new String[] {};
			validateMobGroup(monsters, "natural-monsters",
					new String[] { "PigZombie", "Ghast", "Slime", "Giant", "Spider", "Zombie", "Skeleton", "Creeper" });
			...
			if (!(file.exists())) {
				...
				if (Main.getCRC32("version.txt") == Main.VCRC) {
	...
	private static void validateMobGroup(String[] mobs, String groupname, String[] allowed) {
		lb1: for (String i : mobs) {
			for (String al : allowed)
				if (al.equals(i))
					continue lb1;
			log.warning("Invalid mobType '" + i + "' in group '" + groupname
